...
AlignToReef extends Command{
  ...
  public void initialize(){
     ...
     */
    Transform2d robotToTagOffset = new Transform2d(
    ...
    if(this.alignLeft){
      robotToTagOffset = robotToTagOffset.plus(new Transform2d(0, -Units.inchesToMeters(7.5), Rotation2d.kZero));
    }
    else{
      robotToTagOffset = robotToTagOffset.plus(new Transform2d(0, Units.inchesToMeters(7.5), Rotation2d.kZero));
    }
    ...
    Transform2d fieldRelativeTransform = new Transform2d(
    ...
    SmartDashboard.putNumber("Fancy Math X", robotToTagOffset.getY() * Math.cos(gyroRad) + robotToTagOffset.getX() * Math.sin(gyroRad));
    SmartDashboard.putNumber("Fancy Math Y", robotToTagOffset.getX() * Math.cos(gyroRad) + robotToTagOffset.getY() * Math.sin(gyroRad));

    Rotation2d endRotation = this.drivetrain.getYaw().minus(robotToTagOffset.getRotation()).plus(Rotation2d.kCW_90deg);//.plus(Rotation2d.kCW_90deg);
    ...
    // );
    AutoBuilder.followPath(this.drivetrain.createPathplannerPath(new Transform2d(
        robotToTagOffset.getX(),
        -robotToTagOffset.getY(),
        robotToTagOffset.getRotation()
    ), endRotation)).schedule();
